...
class BootstrapForm extends Form
{
    ...
    public function dropdown($name, array $options, $label)
    {
        $html = ' 
                ...
                <select class="form-select" name=' . $name . ' id="inputGroupSelect01">
        ...
        foreach ($options as $element) {
            ...
            if ((int)$field === $element->id) {
                $selected = "selected";
            }
            $html .= "<option $selected value='$element->id'>$element->title</option>";
        }
        $html .= "</select></div>";
        return $html;
    }
